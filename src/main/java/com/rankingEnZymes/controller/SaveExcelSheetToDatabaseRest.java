...
public class SaveExcelSheetToDatabaseRest {
	...
	public ResponseEntity<?> insertFinalCAZ(UriComponentsBuilder ucBuilder) throws IOException {
	...
	public ResponseEntity<?> insertFungi(UriComponentsBuilder ucBuilder) throws IOException {
	...
	public void readFinalCAZvTypeExcel() throws IOException {
		...
		for (int i = 1; i <= rowCount; i++) {
			...
			if (null != sheet.getRow(i)) {
				...
				Row rowColNames = sheet.getRow(0);
				...
				for (int j = 0; j < maxColumns; j++) {
					...
					try {
						...
						if ((!value.trim().isEmpty()) && value != null) {
							...
							value = value.trim() + "";
							String colName = rowColNames.getCell(j).getStringCellValue();
						...
						for (String s : uniquehemicellulosedegradinglist) {
							//System.out.println("    S-" + s + "-colName-" + colName);
							if ((s.trim().equalsIgnoreCase(colName.trim()))) {
								hemicellulosedegrading = hemicellulosedegrading + (new Long(value)); // Run your code
																										// here
								hemicellulosedegradingCount++;
								break;
							}
